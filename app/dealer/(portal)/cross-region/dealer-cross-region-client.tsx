...
export function DealerCrossRegionClient({ models, initialTransfers, initialCrCaughtRows, hasDealer }: Props) {
  ...
  const [fineState, fineAction, finePending] = useActionState<DealerOutwardState, FormData>(dealerCashFineAction, {});
  const [, startTransition] = useTransition();
  const [editRow, setEditRow] = useState<CrossRegionRow | null>(null);
  const [modelId, setModelId] = useState("");
  const [outwardModelId, setOutwardModelId] = useState("");
  const [fineModelId, setFineModelId] = useState("");
  ...
  useEffect(() => {
    if (fineState.ok) {
      toast.success("Cash fine recorded");
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset the field in response to a server-action result
      setFineModelId("");
      router.refresh();
    } else if (fineState.error) {
      toast.error(fineState.error);
    }
  }, [fineState, router]);
      ...
      {!hasDealer ? (
        ...
        <Tabs defaultValue="inward" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="fine">Cash Fine</TabsTrigger>
          </TabsList>
          ...
          {/* ── CASH FINE (no stock movement) ── */}
          <TabsContent value="fine" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Log a cash fine</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="mb-3 text-xs text-muted-foreground">
                  Records a cash penalty only — no stock is moved. It reduces your net payout for the selected date.
                </p>
                <form action={fineAction} className="grid grid-cols-1 gap-3 sm:grid-cols-5">
                  <input type="hidden" name="modelId" value={fineModelId} />
                  <div className="sm:col-span-2">
                    <Select value={fineModelId} onValueChange={(v) => typeof v === "string" && setFineModelId(v)}>
                      <SelectTrigger className="w-full">
                        <span className={fineModelId ? "" : "text-muted-foreground"}>
                          {models.find((m) => m.id === fineModelId)?.name ?? "Choose model…"}
                        </span>
                      </SelectTrigger>
                      <SelectContent>
                        {activeModels.map((m) => (<SelectItem key={m.id} value={m.id}>{m.name}</SelectItem>))}
                  ...
                  <Input name="fineAmount" type="number" min={1} step="any" placeholder="Fine ₨" required />
                  <Input name="caughtDate" type="date" defaultValue={today} required />
                  <Input name="note" placeholder="Note (optional)" />
                  <Button type="submit" className="sm:col-span-5" disabled={finePending || !fineModelId}>
                    {finePending ? "Saving…" : "Log fine"}
                  </Button>
                </form>
              </CardContent>
            </Card>
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {initialCrCaughtRows.filter((c) => c.quantity === 0).length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={5} className="py-10 text-center text-sm text-muted-foreground">No cash fines logged.</TableCell>
                        </TableRow>
                      ) : initialCrCaughtRows.filter((c) => c.quantity === 0).map((c) => (
                        <TableRow key={c.id}>
                          <TableCell className="font-medium">{c.modelName}</TableCell>
                          <TableCell label="Fine ₨" className="text-right tabular-nums">{c.fineAmount ? formatPKR(c.fineAmount) : "—"}</TableCell>
                          <TableCell label="Date">{formatDate(c.caughtDate)}</TableCell>
                          <TableCell label="Note" className="text-muted-foreground">{c.note ?? "—"}</TableCell>
                          <TableCell>
                            {c.status !== "rejected" ? (
                              <Button size="icon-sm" variant="ghost" aria-label="Delete fine" title="Delete fine" onClick={() => handleDeleteOutward(c.id)}>
                                <Trash2 className="size-4" />
                              </Button>
                            ) : null}
                          </TableCell>
