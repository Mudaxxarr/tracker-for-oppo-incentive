...
export function PurchaseForm({ models, onSuccess }: Props) {
  ...
  const isOverridden =
    !!pricedAt &&
    Number(dealerPrice || 0) !== pricedAt.dealer;
  const isBackDated = purchaseDate < today;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getPriceOnDateAction(modelId, purchaseDate).then((p) => {
      if (cancelled) return;
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      {isOverridden && !isBackDated ? (
        <label className="flex items-start gap-2 rounded-md border bg-muted/30 p-3 text-sm">
          <input
            type="checkbox"
            name="updateMasterPrice"
            value="on"
            className="mt-1 size-4"
          />
          <span>
            Update master dealer price to <strong>{formatPKR(Number(dealerPrice))}</strong> as of
            today? (Future activations will snapshot at the new price.)
          </span>
        </label>
      ) : null}
