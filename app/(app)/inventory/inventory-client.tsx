...
// ─── CARD ─────────────────────────────────────────────────────────────────────
function StockCard({
  row, index, onActivate, onMove, onHistory, onCaught,
}: {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.38, delay: Math.min(index * 0.045, 0.45), ease: [0.22, 1, 0.36, 1] }}
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3 px-4 pt-4 pb-3">
        <div className="min-w-0 flex-1">
          ...
          {value !== null && (
            <p className="mt-0.5 text-xs text-muted-foreground tabular-nums">{formatPKR(value)}</p>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="hidden w-20 sm:block">
        <SourceBar row={row} animate={false} />
      </div>

      {/* Value */}
      {value !== null && (
        <span className="hidden tabular-nums text-xs text-muted-foreground sm:block">
          {formatPKR(value)}
        </span>
      )}
...
function ActivateSheet({ row, open, onClose }: { row: InventoryModelRow | null; open: boolean; onClose: () => void }) {
  ...
  const [state, action, pending] = useActionState<InvActionState, FormData>(quickActivateAction, {});

  // Reset date when sheet opens/closes
  useEffect(() => { if (open) setActivationDate(today); }, [open, today]);

  // Fetch date-bounded stock whenever date or row changes
  useEffect(() => {
    if (!open || !row) { setStockAsOf(null); return; }
    startDateCheck(async () => {
      setStockAsOf(await getStockAsOfAction(row.modelId, activationDate));
    });
              ...
              <div className="mt-2 flex items-center gap-2">
                {dateChecking ? (
                  ...
                  <>
                    ...
                    {isDateConstrained && (
                      <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-950/40 dark:text-amber-400">
                        date-limited
                      </span>
                    )}
                  </>
...
function MoveSheet({ row, open, onClose, otherDealers }: {
  ...
  const [stockAsOf, setStockAsOf] = useState<number | null>(null);
  const [dateChecking, startDateCheck] = useTransition();
  const [state, action, pending] = useActionState<InvActionState, FormData>(quickMoveAction, {});

  useEffect(() => { if (!open) { setToDealerId(""); setTransferDate(today); } }, [open, today]);
  useEffect(() => {
    if (!open || !row) { setStockAsOf(null); return; }
    startDateCheck(async () => {
      setStockAsOf(await getStockAsOfAction(row.modelId, transferDate));
    });
            ...
            {otherDealers.length === 0 ? (
              ...
              <form action={action} className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  The destination ID must accept before stock is added to their inventory.
                </p>
...
function HistorySheet({ row, open, onClose }: { row: InventoryModelRow | null; open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      setEvents(await getModelHistoryAction(row.modelId));
      setLoading(false);
    });
  }, [open, row]);
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-md">
        <SheetHeader>
          <SheetTitle>Stock History — {row?.modelName}</SheetTitle>
        </SheetHeader>
...
function PendingRow({ transfer }: { transfer: PendingTransferRow }) {
  const [acceptState, acceptAction, acceptPending] = useActionState<InvActionState, FormData>(acceptTransferAction, {});
  const [rejectState, rejectAction, rejectPending] = useActionState<InvActionState, FormData>(rejectTransferAction, {});
        ...
        <div className="space-y-2">
          ...
          <AnimatePresence mode="popLayout">
            {pendingTransfers.map((t) => (
              <PendingRow key={t.id} transfer={t} />
            ))}
          </AnimatePresence>
        </div>
      )}
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <AnimatePresence mode="popLayout">
            {filtered.map((row, i) => (
              <StockCard
                key={row.modelId} row={row} index={i}
